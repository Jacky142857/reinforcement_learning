...
def load_all_stock_data(stock_symbols):
    ...
    # Get the parent directory (go up from td3_allocation to root)
...
def train_sb3_portfolio(stock_data_dict, reward_function, max_timesteps=MAX_TRAINING_TIMESTEPS,
    ...
    """Train TD3 agent using Stable Baselines3"""
    ...
    model = TD3(
        ...
        verbose=1,
        tensorboard_log="./td3_portfolio_tensorboard/",
        seed=seed
    )
...
def plot_results(results, reward_name, output_dir):
    ...
    ax.plot(steps, results['portfolio_values'], color='green', linewidth=2.5, label='TD3 Agent', zorder=3)
    ...
    # ========================================================================
    # PLOT 5: TD3 Agent Only (Without Buy & Hold)
    # ========================================================================
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    ...
    plt.tight_layout()
    plot5_path = plots_dir / "5_td3_only.png"
    ...
    while True:
        ...
        if user_input in ['quit', 'q', 'exit']:
            ...
            with open(comparison_path, 'w') as f:
                ...
                f.write(f"{'Rank':<6} {'Reward Function':<40} {'TD3 Return':<12} {'B&H Return':<12} {'Outperf':<10}\n")
                f.write("-"*80 + "\n")
                
                for rank, (name, results) in enumerate(sorted_results, 1):
                    td3_ret = results['total_return']
                    bh_ret = results['test_buy_hold_return']
                    outperf = td3_ret - bh_ret
                    
                    f.write(f"{rank:<6} {name:<40} {td3_ret:>10.2f}% {bh_ret:>10.2f}% {outperf:>+9.2f}%\n")
                    
                    print(f"{rank:<6} {name:<40} TD3: {td3_ret:>7.2f}%  B&H: {bh_ret:>7.2f}%  Δ: {outperf:>+7.2f}%")
