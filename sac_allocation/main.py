...
# MIN_TRAINING_TIMESTEPS = 150_000
# MAX_TRAINING_TIMESTEPS = 750_000
...
class PortfolioGymEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Convert raw action to portfolio weights using a numerically stable softmax
...
class LossVarianceEarlyStopCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        if (self._stable_step_count // self.window_size) >= self.required_stable_windows:
            if self.verbose > 0:
                print(
                    "LossVarianceEarlyStopCallback: stopping training due to sustained low loss variance."
                )
...
def load_all_stock_data(stock_symbols):
    ...
    # Get the parent directory (go up from sac_allocation to root)
...
def plot_results(results, reward_name, output_dir):
    ...
    ax.plot(steps, results['portfolio_values'], color='green', linewidth=2.5, label='SAC Agent', zorder=3)
    ...
    # ========================================================================
    # PLOT 5: SAC Agent Only (Without Buy & Hold)
    # ========================================================================
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    
    steps = range(len(results['portfolio_values']))
    ax.plot(steps, results['portfolio_values'], color='green', linewidth=2.5, label='SAC Agent')
    ...
    ax.set_title(f'SAC Agent Performance - Testing\n({reward_name})', fontweight='bold', fontsize=14)
    ax.set_xlabel('Time Steps', fontsize=12)
    ax.set_ylabel('Portfolio Value ($)', fontsize=12)
    ax.legend(loc='best', fontsize=10)
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plot5_path = plots_dir / "5_sac_only.png"
...
def save_results_summary(results, reward_name, output_dir):
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"SAC Return: {results['total_return']:.2f}%\n")
        ...
        untraded_count = results.get('untraded_stock_count')
        if untraded_count is not None:
            f.write(f"Stocks Not Traded (Testing): {untraded_count}\n")
        f.write("\n")
        
        max_drawdown_pct = calculate_max_drawdown(results.get('portfolio_values', []))
        f.write(f"Max Drawdown (SAC): {max_drawdown_pct:.2f}%\n\n")
...
def run_experiment_for_reward(stock_data_dict, reward_function, output_dir,
    ...
    portfolio_values, allocations, stock_symbols, traded_stocks = test_sb3_portfolio(
        model,
        test_data_dict,
        reward_function,
        seed=seed
    ...
    print(f"  SAC Return: {total_return:.2f}%")
    ...
    while True:
        ...
        if user_input in ['quit', 'q', 'exit']:
            ...
            with open(comparison_path, 'w') as f:
                ...
                f.write(f"{'Rank':<6} {'Reward Function':<40} {'SAC Return':<12} {'B&H Return':<12} {'Outperf':<10}\n")
                f.write("-"*80 + "\n")
                
                for rank, (name, results) in enumerate(sorted_results, 1):
                    sac_ret = results['total_return']
                    bh_ret = results['test_buy_hold_return']
                    outperf = sac_ret - bh_ret
                    
                    f.write(f"{rank:<6} {name:<40} {sac_ret:>10.2f}% {bh_ret:>10.2f}% {outperf:>+9.2f}%\n")
                    
                    print(f"{rank:<6} {name:<40} SAC: {sac_ret:>7.2f}%  B&H: {bh_ret:>7.2f}%  Δ: {outperf:>+7.2f}%")
